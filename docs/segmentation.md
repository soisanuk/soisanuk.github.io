...
**Non-Thai runs split by kind** (`segment.js`). Merging every unknown
character into one blob glued punctuation to the space beside it — `") "`,
`" ("`, `" #"`. Across 3,000 corpus sentences the annotators split all three:
whitespace runs are 72% of non-Thai tokens, latin/digit runs 13.5%, single
marks 7.4%. Runs of the *same* mark stay whole (`...` 217×, `!!!` 48×), and `#`
keeps its latin tag (`#BNK48` 580×). **The single largest change: +5.41 F1.**

**`seg-phrases.js` — 212 entries removed, CC BY-SA 3.0.** Generated by
`scripts/build-seg-phrases.mjs` in two tiers: entries the annotators cut
through at least 20 times and ≥80% of the time, *or* at least 5 times and every
single time. `ตกหนัก` is 0-whole in 24 occurrences. The second tier came later —
หย่ากัน and ตื่นนอน are split 7 of 7 times, under the first bar, and went on
swallowing หย่า and ตื่น in the very sentences that teach them. At 100% you need
fewer sightings to be believed. Measured: 69 entries added, precision unchanged. Its own file
...
| + words and phrases | 89.19 | 87.52 | 88.34 | 35 |
| + non-Thai runs | **89.48** | **98.45** | **93.75** | **97** |
...
- **Thai hashtags are deliberately left apart.** §1.11 keeps `#เรื่องมันช่างน่าอาย`
  whole and for a tokeniser that is right, but this app's reader taps a word to

...
- **The gloss problem (§4 step 5) mostly dissolves.** The worry was that the
  950-word map can't cover open text, so we'd need to ship a dictionary or call
  a lookup service. But the layers this repo uniquely owns — character
  decomposition and tone derivation — work from **spelling alone**. An unknown
  word opens the same card with no gloss and the decomposition auto-expanded,
  and that is genuinely useful on its own. A dictionary would make it better;
